...
<body>
    <nav>
        ...
        $query = "SELECT * FROM admin WHERE uaid = $1";
        ...
        if ($result) {
            ...
            if ($row) {
                ...
                $query1 = "SELECT * FROM course_metadata";
                $result1 = pg_query($db, $query1);
                $courses = pg_fetch_assoc($result1);
            } else {
    ...
    <div id="body">
        ...
        <center id="content">
            <form action="./view_marks_course.php" method="post">
                <h2>Select Course</h2>
                <select name="selectedcourse">
                    <?php
                    if ($result1) {
                        pg_result_seek($result1, 0);
                        while ($course = pg_fetch_assoc($result1)) {
                            echo "<option value='" . $course["course"] . "'>" . $course["name"] . "</option>";
                        }
                    } else {
