...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["submit"])) {
    ...
    if ($result1) {
        ...
        if (empty($row1)) {
            header("Location: ./remove_student.php?DoesNotExist=true");
            exit();
        } else {
            // Insert new student
            $query2 = "DELETE FROM student WHERE usid = $1";
            $result2 = pg_query_params($db, $query2, [$target_usid]);
...
<head>
    ...
    <script>
        function alertInfo() {
            ...
            if (doesnotexist) {
                alert("Cannot Remove Student, ID Does Not Exists");
            }
            return true;
        }
    </script>
