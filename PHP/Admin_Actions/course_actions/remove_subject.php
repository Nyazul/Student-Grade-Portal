...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["submit"])) {
    ...
    if ($result1) {
        ...
        if (empty($row1)) {
            header("Location: ./remove_subject.php?CourseDoesNotExist=true");
            exit();
        } else {
            // remove subject
            $query2 = "ALTER TABLE $course DROP COLUMN $subject";
            $result2 = pg_query($db, $query2);

            if ($result2) {
                header("Location: ./remove_subject.php?RemoveSuccess=true");
                exit();
            } else {
                // Query execution error
                echo "Error: Unable to remove subject.<br>";
...
<head>
    ...
    <script>
        function alertInfo() {
            ...
            const addsuccess = urlParams.get('RemoveSuccess');
            ...
            if (doesnotexist) {
                alert("Cannot Remove Subject, Course Does Not Exist");
            }
            return true;
        }
    </script>
